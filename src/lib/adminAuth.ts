...
export async function verifyAndConsumeCode(code: string) {
  const hash = hashCode(code);
  const rows = await sql`SELECT * FROM admin_login_codes WHERE code_hash = ${hash} AND used_at IS NULL AND expires_at > now() ORDER BY id DESC LIMIT 1` as any[];
...
export function verifyAdminToken(token: string | undefined) {
  if (!token) return false; try { return (jwt.verify(token, getSecret()) as any).role === 'admin'; } catch { return false; }
}
